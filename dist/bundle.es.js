...
const mount$$1 = mount$1;

const update$$1 = update$1;
...
var reduxQuerySync = createCommonjsModule(function (module, exports) {
'use strict';
...
var ReduxQuerySync = unwrapExports(reduxQuerySync);
...
var reduxLogger$1 = createCommonjsModule(function (module, exports) {
...
var reduxLogger_1 = reduxLogger$1.createLogger;

var logger = reduxLogger_1({
	timestamp: false,
	collapsed: (getState, action, logEntry) => !logEntry.error
});

const getFocus = state =>
	state.focus;
...
store.subscribe(update$$1);

const dispatchInit = compose(
	store.dispatch,
	action.init
);

const dispatchSetFocus = compose(
	store.dispatch,
	action.setFocus
);
...
riot$1.tag2('buttons-bar', '<button aria-label="1" class="button-one {selected: oneOnFocus}" onclick="{setFocusOne}">1</button> <button aria-label="2" class="button-two {selected: twoOnFocus}" onclick="{setFocusTwo}">2</button> <button aria-label="3" class="button-three {selected: threeOnFocus}" onclick="{setFocusThree}">3</button>', 'buttons-bar .selected,[data-is="buttons-bar"] .selected{ border: 2px solid red; }', '', function(opts) {
